...
import {GradeSheetCourseInfo, GradeSheetOverallResult, GradeSheetParsedData, GradeSheetSemester, GradeSheetSemesterResult} from "@/types/usisTypes"
import {AxiosInstance} from "axios";
import { gradeSheetEndpoint } from "./usisApiRoutes";

export async function getGradeSheetData(client:AxiosInstance): Promise<GradeSheetParsedData> {
...
function parseGradeSheet(htmlString: string): GradeSheetParsedData {
    ...
    resultTable.find('tr').each((index, row) => {
      ...
      if (firstCellText !== "Course No") {
        if (cells.length === 2 && firstCellText.startsWith("SEMESTER")) {
        ...
        } else if (cells.length === 6 && !firstCellText.startsWith("SEMESTER") && !firstCellText.startsWith("CUMULATIVE")) {
          ...
          const semesterResult: GradeSheetSemesterResult = {
            creditsAttemped: parseFloat(
              cells.eq(1).text().match(/\d+\.\d+/)[0]
            ),
